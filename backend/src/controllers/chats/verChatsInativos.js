...
const verChatsInativos = async (req, res) => {
	...
	db.query(sql, [userId, userId], (err, results) => {
		if (err) {
			return res
				.status(500)
				.json({
					error: "Erro ao buscar chats inativos: " + err.message,
				});
		}
		...
		if (results.length === 0) {
			return res
				.status(200)
				.json({ message: "Não há chats inativos no momento." });
		}
...
module.exports = verChatsInativos;
